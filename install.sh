...
if [ "$1" != "-yt" ]; then  # if you don't want to install yt-dlp, use the -yt flag
    ...
    curl https://gist.githubusercontent.com/carlosaugustoveras/b56067968e154f16bfd1af6bde18e929/raw/8b32b4390983359b60941f429083daf5ee51aacf/yt-dlp.sh | bash
fi

# check if current folder is not a git repository
if [ ! -d .git ]; then
    echo "Baixando bot..."
    git clone https://github.com/carlosaugustoveras/WhatsappBot.git 2>&1>/dev/null
    cd WhatsappBot
fi
